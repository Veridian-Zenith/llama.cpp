...
void * ggml_sycl_malloc_device(size_t size, sycl::queue &q, ggml_sycl_mem_type type) {
    ...
    if (ggml_sycl_use_level_zero_device_alloc(q)) {
        // USER FIX: integrated GPU (Alder Lake iGPU 8086:46B3); zeMemAllocDevice
        // exhausts driver tracking (xe ENOSPC -> VM_FAULT_OOM). Use shared alloc.
        if (q.get_device().has(sycl::aspect::ext_oneapi_is_integrated_gpu)) {
            void *ptr = sycl::malloc_shared(size, q);
            if (ptr) { ggml_sycl_memtrace_add(type, ptr, size); return ptr; }
            GGML_LOG_WARN("%s: malloc_shared failed, fallback malloc_device", __func__);
